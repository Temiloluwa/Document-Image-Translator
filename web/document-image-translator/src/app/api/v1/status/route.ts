...
export async function GET(req: NextRequest) {
  ...
  const apiUrl = `${process.env.BACKEND_API_URL}/v1/status?${param}=${encodeURIComponent(value)}`;
  try {
    ...
    console.log("Lambda /v1/status response:", data);
    if (!res.ok) {
      console.error("Backend error (status)", { status: res.status, data });
    }
    return new Response(JSON.stringify(data), { status: res.status });
  } catch (err) {
    console.error("Fetch error (status)", err);
